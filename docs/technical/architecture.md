...
- `js/services/progression-section-document-service.js`: operaciones estructurales de secciones, como localizar, clonar, anexar, anotar medidas, calcular rangos y describir transiciones entre sección de origen y destino.
- `js/services/progression-section-retarget-service.js`: sustitución del contexto tonal de una sección existente conservando contrato temporal, metadatos de sección y limpieza de modulaciones obsoletas.
...
- `js/services/progression-section-modulation-service.js`: preparación de transiciones modulantes entre secciones, cálculo de acordes pivote y aplicación de acordes de transición sin mezclarlo con la UI.
...
- El contrato público de documentos, secciones, compases y segmentos de progresión vive en `docs/technical/progression-segment-contract.md`. Los metadatos no enumerables de algoritmos internos no deben convertirse en dependencias de UI, persistencia, playback ni exportación MIDI.
- Todo cálculo o copia de campos temporales de progresión debe pasar por `CodaProgressionTiming` o por servicios que lo usen internamente. Evitar duplicar reglas sobre `startBeat`, `startSeconds`, `durationBeats`, `durationSeconds`, `secondsPerBeat` y `beatsPerBar`.
...
- La lógica de secciones debe mantenerse separada por responsabilidad: documento de secciones, retarget tonal, variaciones, candidatos tonales, modulación, operaciones de alto nivel y orquestación. Evitar que `progression-section-contrast-service.js` vuelva a acumular clonación, reindexado, variación, búsqueda tonal y reglas modulantes en un único archivo.
- Cualquier consulta sobre rangos, sección anterior, sección posterior o transición entre secciones debe pasar por `CodaProgressionSectionDocument`. Evitar cálculos locales de `startIndex + length` dispersos en análisis, renderers o controladores.
- Las etiquetas analíticas visibles deben derivarse desde `CodaProgressionAnalysisLabels` o servicios equivalentes a partir del documento final. El renderizado no debe decidir por sí solo si un pivote, dominante secundaria u otro recurso sigue siendo válido.
- El contrato visible de etiquetas analíticas usa descriptores de `CodaProgressionAnalysisLabels.sourceLabelDescriptor`: tipo, texto, visibilidad y fuente normalizada. Los renderers pueden mostrar el texto, pero no reconstruir el significado analítico de pivotes, dominantes secundarias, intercambio o cromatismo.
...
- La vista de instrumento usa alturas MIDI explícitas en notación científica estándar: C4 es el C central y equivale a la nota MIDI 60. El teclado se construye en HTML y CSS desde A0 hasta C8, las 88 teclas del piano estándar. La guitarra también se construye sin imagen de fondo y muestra 24 trastes; la afinación estándar parte de 6ª E2, 5ª A2, 4ª D3, 3ª G3, 2ª B3 y 1ª E4.
...
- Si se añade un control de generación de progresiones o secciones, debe ampliarse `js/ui/progression-generation-events-controller.js` y cubrirse con pruebas unitarias de servicios o arquitectura.
