...
La parsimonia se evalúa por voz, no sólo por distancia total del acorde. Los movimientos por semitono, tono o tercera deben prevalecer siempre que el acorde lo permita; las cuartas, quintas y saltos mayores reciben una penalización creciente, con especial cuidado en la voz superior. Si dos candidatos tienen un coste global parecido, se prefiere el que reparte movimientos pequeños entre las voces antes que el que deja varias voces quietas y concentra un salto amplio en soprano o en una voz interior. Cuando una progresión exige un salto, el generador debe preferir que lo asuma el bajo o una voz inferior antes que la soprano; la voz superior se percibe como línea melódica principal y sus saltos amplios deben ser excepcionales, preferentemente compensados después por movimiento contrario y cercano. Las disposiciones abiertas pueden separar el registro, pero no deben aislar una voz superior a una distancia exagerada si eso rompe la identidad melódica de las líneas.
...
El espaciado grave no debe convertirse en una excusa para bajar todo el bajo una octava. Cuando el registro de partida ya es grave, la separación se fuerza por claridad instrumental; cuando la opción grave aparece sólo como candidato alternativo, compite contra la versión más cercana y recibe penalización si sacrifica la conducción. Así se evita el caso de un acorde correcto en notas pero escrito como bloque desplazado hacia abajo, con bajo excesivamente bajo y voces superiores desconectadas.
...
La puntuación añade una capa de instrumentación idiomática. En guitarra clásica, cada nota simultánea debe asignarse a una cuerda física distinta dentro del rango real del instrumento; no se admite como digitación válida que una misma cuerda suene al aire y pisada, ni que se pisen dos trastes de la misma cuerda dentro del mismo acorde. Se favorecen cuerdas al aire, bajos en cuerdas graves, digitaciones compactas y cejillas, y se penalizan con fuerza las aperturas que exceden la mano izquierda. La vista de diapasón aplica la misma idea al iluminar notas: escoge una digitación coherente para el acorde completo, no posiciones aisladas nota a nota. En piano acústico, el acorde se evalúa como un reparto entre dos manos: hasta la octava se considera seguro, la novena recibe penalización suave, la décima penalización clara y cualquier apertura mayor se trata como escritura que debería redistribuirse o arpegiarse. Estas reglas no bloquean la creatividad, pero hacen que el selector prefiera voicings tocables cuando existen alternativas equivalentes.
