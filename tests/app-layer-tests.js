...
assert.deepEqual(generatedHighColorProgression.measures[0].pedalsOut.map(function (pedal) { return pedal.note; }).sort(), ['C', 'G']);
assert.ok(/^6(\/4)?$/.test(generatedHighColorProgression.measures[1].inversion));
assert.ok(generatedHighColorProgression.measures[1].suspension === '' || generatedHighColorProgression.measures[1].suspension === 'sus2');
assert.ok(/^iv 6(\/4)?( sus2)?$/.test(generatedHighColorProgression.measures[1].degree));
assert.ok(/^Fm 6(\/4)?/.test(generatedHighColorProgression.measures[1].displayName));
assert.ok(/add(11|13)/.test(generatedHighColorProgression.measures[1].displayName));
assert.deepEqual(generatedHighColorProgression.measures[1].pedalsIn.map(function (pedal) { return pedal.midiNote; }).sort(function (a, b) { return a - b; }), [60, 67]);
assert.equal(generatedHighColorProgression.measures[1].voiceLeading.parallelPerfects, 0);
assert.ok(generatedHighColorProgression.measures[2].displayName.indexOf('Fm7') === 0);
